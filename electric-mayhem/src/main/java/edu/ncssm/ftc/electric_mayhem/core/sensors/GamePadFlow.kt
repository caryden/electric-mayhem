...
class GamePadFlow(private val gamepad : Gamepad) {
    ...
    val rightTrigger = edu.ncssm.ftc.electric_mayhem.core.sensors.Trigger({ gamepad.right_trigger }).flow
    val leftTrigger = edu.ncssm.ftc.electric_mayhem.core.sensors.Trigger({ gamepad.left_trigger }).flow

    val rightJoystick = Joystick({ JoystickData(gamepad.right_stick_x, gamepad.right_stick_y) }).flow
    val leftJoystick = Joystick({ JoystickData(gamepad.right_stick_x, gamepad.right_stick_y) }).flow
